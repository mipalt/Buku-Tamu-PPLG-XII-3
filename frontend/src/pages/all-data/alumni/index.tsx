...
const AlumniPage: React.FC = () => {
  ...
  const { data, error, isLoading } = useFetch<ApiResponse>(
    `http://localhost:8000/api/guest-alumni?page=${currentPage}&per_page=${rowsPerPage}`,
    token
  );
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setSelectedIds(currentRows.map((alumni) => alumni.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const performDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/guest-alumni/${deleteId}`, {
        method: "DELETE",
        headers: {
      ...
      const updatedData = filteredData.filter((alumni) => alumni.id !== deleteId);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `guest_alumni.${exportFormat.toLowerCase()}`;
  ...
  const handleView = (id: number) => {
    console.log(`View alumni with id: ${id}`);
  };
  ...
  return (
   ...
   <thead className="bg-[#F4F7FC] border-b border-gray-200">
     ...
     <tr>
       ...
       <th
         className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
         onClick={() => handleSort("alumni_name")}
         aria-sort={sortColumn === "alumni_name" ? sortDirection : "none"}
       >
         <div className="flex items-center space-x-1">
           <span>NAMA</span>
           {sortColumn === "name" && (
       ...
       <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
        AKSI
             ...
             <tbody className="divide-y divide-gray-200">
               {currentRows.map((alumni, index) => (
                 <tr key={alumni.id} className="even:bg-gray-50 hover:bg-gray-100">
                   <td className="px-6 py-4 whitespace-nowrap">
                     <input
                       type="checkbox"
                       className="rounded border-gray-300"
                       checked={selectedIds.includes(alumni.id)}
                       onChange={(e) => handleCheckboxChange(alumni.id, e.target.checked)}
                     />
                   </td>
                   ...
                   <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {new Date(alumni.created_at).toLocaleDateString("id-ID")}
                   </td>
