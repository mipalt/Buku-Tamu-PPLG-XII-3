...
interface GuestVisitor {
  ...
  institution: string; // Nama perusahaan
  phone: string;
  email: string;
...
const Visitor: React.FC = () => {
  ...
  const [sortColumn, setSortColumn] = useState<"id" | "institution" | null>(null);
  ...
  const { data, error, isLoading } = useFetch<ApiResponse>(
    `http://localhost:8000/api/guest-visitors?page=${currentPage}&per_page=${rowsPerPage}`,
    token
  );
  ...
  const filteredData = data?.data?.filter(visitor =>
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    const aValue = sortColumn === "id" ? a.id : a.institution?.toLowerCase() || "";
    const bValue = sortColumn === "id" ? b.id : b.institution?.toLowerCase() || "";
  ...
  const handleSort = (column: "id" | "institution") => {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setSelectedIds(currentRows.map((visitor) => visitor.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const performDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/guest-visitors/${deleteId}`, {
        method: "DELETE",
        headers: {
      ...
      const updatedData = filteredData.filter((visitor) => visitor.id !== deleteId);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `guest_visitors.${exportFormat.toLowerCase()}`;
  ...
  const handleView = (id: number) => {
    console.log(`View visitor with id: ${id}`);
  };
            ...
            <thead className="bg-[#F4F7FC] border-b border-gray-200">
              ...
              <tr>
                ...
                <th
                  ...
                  aria-sort={
                    sortColumn === "institution" ? sortDirection : "none"
                  }
                >
                  <div className="flex items-center space-x-1">
                    <span>NAMA</span>
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      ...
                      onChange={(e) =>
                        handleCheckboxChange(visitor.id, e.target.checked)
                      }
                    />
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-sm text-gray-700">
                    {visitor.institution || "-"}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(
                        visitor.purposes?.purpose || "UNKNOWN"
                      )}`}
                    >
                      {visitor.purposes?.purpose || "-"}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    {visitor.created_at
                      ? new Date(visitor.created_at).toLocaleDateString("id-ID", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                        })
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                    <button
                      ...
                      aria-label={`Hapus ${visitor.institution || "perusahaan"}`}
                    >
                    ...
                    <button
                      ...
                      aria-label={`Lihat detail ${
                        visitor.institution || "perusahaan"
                      }`}
                    >
