...
interface GuestParent {
  id: number;
  name: string; // Nama orang tua
  rayon: string;
  student_name: string; // Orang tua dari
  phone: string;
  email: string | null;
  address: string;
...
const Parent: React.FC = () => {
  ...
  // Mengambil data dari endpoint /api/guest-parents
  const { data, error, isLoading } = useFetch<ApiResponse>(
    `http://localhost:8000/api/guest-parents?page=${currentPage}&per_page=${rowsPerPage}`,
    token
  );
  ...
  const getStatusColor = (purpose: string) => {
    ...
    } else if (purpose.toUpperCase().includes("KONSULTASI")) {
  ...
  const filteredData = data?.data?.filter(parent =>
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    const aValue = sortColumn === "id" ? a.id : a.name?.toLowerCase() || "";
    const bValue = sortColumn === "id" ? b.id : b.name?.toLowerCase() || "";
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      setSelectedIds(currentRows.map((parent) => parent.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const performDelete = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/guest-parents/${deleteId}`, {
        method: "DELETE",
        headers: {
      ...
      const updatedData = filteredData.filter((parent) => parent.id !== deleteId);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to export data: ${response.status}`);
      }
      ...
      a.href = url;
      a.download = `guest_parents.${exportFormat.toLowerCase()}`;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full table-auto lg:table-fixed">
          ...
          <tbody className="divide-y divide-gray-200">
            {currentRows.map((parent) => (
              <tr key={parent.id} className="even:bg-gray-50 hover:bg-gray-100">
                <td className="px-6 py-4 whitespace-nowrap">
                  <input
                    type="checkbox"
                    className="rounded border-gray-300"
                    checked={selectedIds.includes(parent.id)}
                    onChange={(e) => handleCheckboxChange(parent.id, e.target.checked)}
                  />
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`inline-flex px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(
                      parent.purposes?.purpose || "UNKNOWN"
                    )}`}
                  >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                  {parent.created_at ? new Date(parent.created_at).toLocaleDateString("id-ID") : "-"}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  <button
                    className="text-red-600 hover:text-red-800 p-1 rounded"
                    onClick={() => confirmDelete(parent.id)}
                    aria-label={`Hapus ${parent.name || "orang tua"}`}
                  >
                  ...
                  <button
                    ...
                    aria-label={`Lihat detail ${parent.name || "orang tua"}`}
                  >
